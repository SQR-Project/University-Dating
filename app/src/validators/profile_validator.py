...
def validate(request: CreateProfileRequest) -> bool:
    if request.age <= 10 or request.age > 100:
        raise HTTPException(
            status_code=400,
            detail="CreateProfileRequest validation error. Age should be greater than 10 amd less than 100"
        )
    ...
    if match_name is None or match_surname is None:
        raise HTTPException(
            status_code=400,
            detail="CreateProfileRequest validation error. Name and surname should consist of: letters, -, ., and '"
        )
    return True
