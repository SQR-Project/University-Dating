...
def test_like_profile_db_throws_integrity_error_http_exception(mock_db):
    # Arrange
    mock_db.return_value.get_profile_likes_by_user_id.side_effect = (
        IntegrityError()
    )

    # Act
    with pytest.raises(Exception) as exc_info:
        like_service.like_profile(
            VALID_TOKEN_DATA,
            VALID_LIKE_PROFILE_REQUEST
        )
...
def test_like_profile(mock_db):
    ...
    # Act
    response = like_service.like_profile(
        VALID_TOKEN_DATA,
        VALID_LIKE_PROFILE_REQUEST
    )
...
def test_is_matched(mock_db):
    ...
    mock_db.return_value.unsafe_get_profile_by_email.return_value = [
        ("test_id")
    ]
    mock_db.return_value.get_profile_likes_by_user_id.return_value = [
        ("email")
    ]

    # Act
    response = like_service.is_matched(
        VALID_TOKEN_DATA,
        VALID_LIKE_PROFILE_REQUEST
    )
...
def test_is_matched_db_throws_integrity_error_http_exception(mock_db):
    ...
    mock_db.return_value.unsafe_get_profile_by_email.side_effect = (
        IntegrityError()
    )

    # Act
    with pytest.raises(Exception) as exc_info:
        like_service.is_matched(
            VALID_TOKEN_DATA,
            VALID_LIKE_PROFILE_REQUEST
        )
